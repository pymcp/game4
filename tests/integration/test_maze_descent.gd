...
func test_maze_entrance_enter_and_floor1() -> void:
	var rid := Vector2i(0, 0)
	var cell := Vector2i(20, 20)
	var mid: StringName = MapManager.make_id(rid, cell, 1, &"maze")
	var m: InteriorMap = MapManager.get_or_generate(mid, rid, cell, 1, 64, &"maze")
	assert_not_null(m)
	assert_eq(m.floor_num, 1)
	assert_true(m.width >= 16 and m.width <= 96)
	assert_eq(MapManager._kind_from_id(m.map_id), &"maze")


func test_maze_descent_preserves_kind() -> void:
	var rid := Vector2i(0, 0)
	var cell := Vector2i(30, 30)
	var mid: StringName = MapManager.make_id(rid, cell, 1, &"maze")
	var floor1: InteriorMap = MapManager.get_or_generate(mid, rid, cell, 1, 64, &"maze")
	var floor2: InteriorMap = MapManager.descend_from(floor1, 64)
	assert_eq(floor2.floor_num, 2)
	assert_eq(MapManager._kind_from_id(floor2.map_id), &"maze")
	var floor3: InteriorMap = MapManager.descend_from(floor2, 64)
	assert_eq(floor3.floor_num, 3)
	assert_eq(MapManager._kind_from_id(floor3.map_id), &"maze")


func test_maze_has_chest_scatter() -> void:
	var rid := Vector2i(0, 0)
	var cell := Vector2i(40, 20)
	var mid: StringName = MapManager.make_id(rid, cell, 1, &"maze")
	var m: InteriorMap = MapManager.get_or_generate(mid, rid, cell, 1, 64, &"maze")
	assert_true(m.chest_scatter.size() >= 1, "Floor 1 maze should have chest(s)")
...
func test_boss_floor_has_boss_data() -> void:
	...
	var mid: StringName = MapManager.make_id(rid, cell, 2, &"maze")
	var m: InteriorMap = MapManager.get_or_generate(mid, rid, cell, 2, 64, &"maze")
	assert_false(m.boss_data.is_empty(), "Floor 2 maze should have boss_data")
...
func test_maze_map_is_connected() -> void:
	var rid := Vector2i(0, 0)
