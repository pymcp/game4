...
func test_tres_loads_and_has_expected_keys() -> void:
	...
	# Maze fields must be present and non-empty after reseeding.
	assert_true(loaded.maze_terrain.has(&"floor"), "maze_terrain must have 'floor' key")
	assert_true(loaded.maze_terrain.size() > 0, "maze_terrain non-empty")
	assert_true(loaded.maze_wall_autotile.size() > 0, "maze_wall_autotile non-empty")
	assert_true(loaded.maze_floor_decor.size() > 0, "maze_floor_decor non-empty")
	assert_true(loaded.dungeon_floor_border_3x3.size() == 9,
		"dungeon_floor_border_3x3 size == 9")
	assert_true(loaded.maze_floor_border_3x3.size() == 9,
		"maze_floor_border_3x3 size == 9")
