...
func test_generates_interiormap() -> void:
	var m: InteriorMap = MazeGenerator.generate(12345, 64, 64, 1)
	assert_not_null(m)
...
func test_entry_and_exit_are_floor() -> void:
	var m: InteriorMap = MazeGenerator.generate(42, 64, 64, 1)
...
func test_maze_is_connected() -> void:
	var m: InteriorMap = MazeGenerator.generate(99, 64, 64, 1)
...
func test_has_chests_at_dead_ends() -> void:
	var m: InteriorMap = MazeGenerator.generate(7777, 64, 64, 1)
...
func test_no_boss_on_non_boss_floor() -> void:
	...
	var m: InteriorMap = MazeGenerator.generate(1, 64, 64, 1)
	assert_true(m.boss_data.is_empty(), "Floor 1 should have no boss")


func test_boss_on_boss_floor() -> void:
	var interval: int = EncounterTableRegistry.get_boss_interval(&"maze")
	var m: InteriorMap = MazeGenerator.generate(1, 64, 64, interval)
...
func test_variable_size() -> void:
	var m_small: InteriorMap = MazeGenerator.generate(1, 10, 10, 1)
	assert_eq(m_small.width, InteriorMap.MIN_SIZE)
	var m_big: InteriorMap = MazeGenerator.generate(1, 200, 200, 1)
