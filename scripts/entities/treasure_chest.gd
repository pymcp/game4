...
## Fixed loot to give when opened. Each entry: {id: StringName, count: int}.
## When empty, falls back to ChestLootRegistry random rolls.
@export var fixed_loot: Array = []
## Quest integration: if non-empty, mark this objective done on open.
@export var quest_id: StringName = &""
@export var quest_objective_id: StringName = &""
...
func open(player: Node) -> void:
	...
	# Advance quest objective if set (before spawning pickups, so collect
	# notifications from LootPickup don't race with the objective state).
	if quest_id != &"" and quest_objective_id != &"":
		QuestTracker.mark_objective_done(String(quest_id), String(quest_objective_id))
	# Spawn fixed loot or fall back to random registry rolls.
	if not fixed_loot.is_empty():
		for entry in fixed_loot:
			var iid: StringName = StringName(entry.get("id", &""))
			if iid == &"":
				continue
			var pickup := LootPickup.new()
			pickup.item_id = iid
			pickup.count = int(entry.get("count", 1))
